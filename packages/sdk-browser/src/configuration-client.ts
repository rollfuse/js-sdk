...
export class ConfigurationClient {
   ...
   * lower/equal version is always a no-op. JS is single-threaded, so a
   * plain compare-then-assign is sufficient.
   */
  ...
  stop(): void {
    ...
    // reader.read() rejects with an AbortError, treated like any other
    // ended connection) and a currently-pending reconnect backoff sleep,
    // so streamLoop observes `this.stopped` and exits promptly instead of
    // outliving stop() by up to STREAM_MAX_BACKOFF_MS.
  ...
  private currentRefreshIntervalMs(): number {
    ...
    // own hello event disclosed wins (task 5.4) — see the sibling
    // decision in @rollfuse/sdk-js's identical method.
   ...
   * outright. Every failure path is reported through onConfigRefreshError
  ...
  private async streamConnectOnce(): Promise<void> {
    ...
    try {
      ...
      // this connection existed — wake it so it recomputes immediately
      // using the now-known reduced interval (task 5.4).
      this.wakePollLoop();
   ...
   * (tasks 5.5/5.6 — see the sibling implementation in @rollfuse/sdk-js
  ...
  /** Cancels a pending poll timer and runs the loop immediately. A no-op if no timer is pending (a fetch is already in flight, or the loop hasn't started yet). */
