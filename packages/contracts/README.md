...
To ship a change to consumers: bump `version` in `package.json` (`files`
is `["dist"]` only — `src/` is not published, keeping the tarball small),
`npm run build`, `npm publish`, then in each consumer bump the declared
`@rollfuse/contracts` range and run `npm install`.
